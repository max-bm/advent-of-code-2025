...
def parseInstructions(input: Seq[String]): Seq[Int] =
  input.map(parseInstruction).toSeq
...
def part_2(instructions: Seq[Int]): Int =
  var pos: Int = 50
  val flatInstructions: Seq[Int] =
    instructions.flatMap(i => Seq.fill(i.abs)(i.sign))
